...
describe('AuthGuard', () => {
    ...
    beforeEach(() => {
        const authServiceSpy = jasmine.createSpyObj('AuthService', ['isTokenValid']);
    ...
    it('should allow activation when token is valid', async () => {
        const response: ApiAuthResponse = { code: 'success', message: 'Token is valid' };
    ...
    it('should prevent activation and redirect to login when token is invalid', async () => {
        const response: ApiAuthResponse = { code: 'error', message: 'Token is invalid' };
        authService.isTokenValid.and.returnValue(Promise.resolve(response));
